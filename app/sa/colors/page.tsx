...
export default function ColorsPage() {
    ...
    async function load() {
        setLoading(true);
        try {
            const data = await apiFetch<ColorModel[]>("/colors");
            setColors(data);
        } catch (e: any) {
    ...
    async function onCreate(e: React.FormEvent) {
        ...
        setCreateMsg(null);
        try {
            await apiFetch("/colors", {
                method: "POST",
                body: JSON.stringify({ name: newName }),
            });
            setNewName("");
            setCreateMsg("Color creado correctamente ✅");
            load();
        } catch (e: any) {
    ...
    function startEdit(t: ColorModel) {
        setEditingId(t.id);
        setEditName(t.name);
    }
    ...
    async function deleteItem(id: string) {
        if (!confirm("¿Seguro que deseas eliminar este color? No podrás hacerlo si ya está en uso.")) return;
        setDeletingId(id);
        try {
                            ...
                            <div className="flex items-center gap-3 mb-4">
                                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-pink-100 text-pink-600 dark:bg-pink-900/30 dark:text-pink-400">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="h-6 w-6">
                                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.974 0-5.749.534-8.161 1.49m16.322 0A11.954 11.954 0 0112 14.5c-2.924 0-5.637-.474-8.03-1.314" />
                                    </svg>

                                </div>
                                <div>
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white uppercase tracking-tight">Nuevo Color</h3>
                                    <p className="text-xs text-slate-500 dark:text-slate-400">Administrar catálogo</p>
                                </div>
                            </div>
                ...
                <div className="flex-1 min-w-0">
                    <div className="overflow-hidden rounded-xl bg-white shadow-md border border-slate-200 dark:bg-slate-800 dark:border-slate-700">
                        <div className="border-b border-slate-200 bg-white px-6 py-4 dark:bg-slate-800 dark:border-slate-700 flex justify-between items-center">
                            <div>
                                <h3 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tight">Catálogo de Colores</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400">Colores disponibles para asignar a vehículos.</p>
                            </div>
                            <button
                                onClick={load}
                        ...
                        <ul className="divide-y divide-slate-100 dark:divide-slate-700">
                            ...
                            {colors.length === 0 && !loading && (
                                <li className="p-8 text-center text-sm text-slate-500 uppercase tracking-widest font-medium">
                                    Aún no hay colores registrados.
                                </li>
                            )}
                        </ul>
                    </div>
